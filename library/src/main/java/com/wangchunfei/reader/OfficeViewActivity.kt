package com.wangchunfei.reader
...
import kotlinx.android.synthetic.main.activity_office_view.mOfficeView

open class OfficeViewActivity : AppCompatActivity() {
    private val TAG = "OfficeViewActivity"

    companion object {
        fun launchOfficeView(
            activity: AppCompatActivity, docSourceType: Int, path: String?,
            fileType: Int? = null, engine: Int? = null
        ) {
            var intent = Intent(activity, OfficeViewActivity::class.java)
    ...
    fun initData(intent: Intent?) {
        ...
        mOfficeView.openDoc(this,docUrl,docSourceType,fileType,false, DocEngine.values().first { it.value == engine })
