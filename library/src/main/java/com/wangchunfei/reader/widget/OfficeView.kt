package com.wangchunfei.reader.widget
...
import com.wangchunfei.reader.R
import com.wangchunfei.reader.bean.DocEngine
import com.wangchunfei.reader.bean.DocMovingOrientation
...
import com.wangchunfei.reader.pdf.PdfDownloader
import com.wangchunfei.reader.pdf.PdfPageViewAdapter
import com.wangchunfei.reader.pdf.PdfQuality
import com.wangchunfei.reader.pdf.PdfRendererCore
import com.wangchunfei.reader.pdf.PdfViewAdapter
import com.wangchunfei.reader.util.Constant
import com.wangchunfei.reader.util.FileUtils
import com.wangchunfei.reader.util.ViewUtils.hide
import com.wangchunfei.reader.util.ViewUtils.show
import kotlinx.android.synthetic.main.office_view.view.*
import java.io.File
...
class OfficeView : FrameLayout, OnDownloadListener, OnWebLoadListener, OnPdfItemClickListener {

    private val TAG = "OfficeView"
    ...
    fun initView(attrs: AttributeSet?, defStyle: Int) {
        inflate(context, R.layout.office_view, this)
        ...
        val typedArray =
            context.obtainStyledAttributes(attrs, R.styleable.OfficeView, defStyle, 0)
        val orientation =
            typedArray.getInt(R.styleable.OfficeView_dv_moving_orientation, DocMovingOrientation.HORIZONTAL.orientation)
        mMovingOrientation = DocMovingOrientation.values().first { it.orientation == orientation }
        val ratio =
            typedArray.getInt(R.styleable.OfficeView_dv_quality, PdfQuality.NORMAL.ratio)
        quality = PdfQuality.values().first { it.ratio == ratio }
        val engineValue =
            typedArray.getInt(R.styleable.OfficeView_dv_engine, DocEngine.INTERNAL.value)
        engine = DocEngine.values().first { it.value == engineValue }
        showDivider = typedArray.getBoolean(R.styleable.OfficeView_dv_showDivider, true)
        showPageNum = typedArray.getBoolean(R.styleable.OfficeView_dv_show_page_num, true)
        divider = typedArray.getDrawable(R.styleable.OfficeView_dv_divider)
        enableLoadingForPages = typedArray.getBoolean(R.styleable.OfficeView_dv_enableLoadingForPages, enableLoadingForPages)
        pbHeight = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_pb_height, pbDefaultHeight)
        pbColor = typedArray.getColor(R.styleable.OfficeView_dv_page_pb_color, pbDefaultColor)

        val marginDim = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_margin, 0)
        pageMargin = Rect(marginDim, marginDim, marginDim, marginDim).apply {
            top = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_marginTop, top)
            left = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_marginLeft, left)
            right = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_marginRight, right)
            bottom = typedArray.getDimensionPixelSize(R.styleable.OfficeView_dv_page_marginBottom, bottom)
        }
